class ValidateEmailController < ApplicationController
  def new
    ...
    if @user
      if BCrypt::Password.new(@user.email_validation_token).is_password?(session[:email_valid_token])
        @user.email_validation_sent_at = Time.zone.now
        if @user.save
          EmailValidationMailer.send_email_validation_to_user(@user, @user.gen_email_validation_link(request.base_url, session[:email_valid_token], @user)).deliver_later
    ...
    if @user
      if BCrypt::Password.new(@user.email_validation_token) != params[:id]
        ...
        if @user.update_attribute("valid_email", true)
          @user.update_attribute("email_validation_token", '')
